      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.campoIdentificador}
              onChange={(e) => setFormData({ ...formData, campoIdentificador: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            />
          </div>
        ...
        {exams.map((exam) => {
          ...
          return (
            <EntityCard
              key={exam._id}
              title={exam.titulo}
              subtitle={`Turma: ${examClass?.titulo || "N/A"}`}
              description={`Peso: ${exam.peso} | Campo: ${exam.campoIdentificador}`}
              actions={
                <button
